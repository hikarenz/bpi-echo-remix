-- First, create the missing helper function
...
-- Drop ALL existing policies on vendor_companies
DROP POLICY IF EXISTS "Admins can create vendor companies" ON public.vendor_companies;
DROP POLICY IF EXISTS "Admins can manage all vendor companies" ON public.vendor_companies;
DROP POLICY IF EXISTS "Authenticated users can create vendor companies" ON public.vendor_companies;
...
DROP POLICY IF EXISTS "Vendors can update their own company" ON public.vendor_companies;
DROP POLICY IF EXISTS "Vendors can view their own company" ON public.vendor_companies;

-- Drop ALL existing policies on vendor_users
...
DROP POLICY IF EXISTS "Users can view their own vendor association" ON public.vendor_users;
DROP POLICY IF EXISTS "Vendors can create their own user association" ON public.vendor_users;

-- Create simplified and working RLS policies for vendor_companies
...
-- Create simplified and working RLS policies for vendor_users
